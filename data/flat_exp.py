...
def ner_t5_data_format(fout, item, schemas):
    global recall_model
    entity_set = copy.deepcopy(schemas)

    entity_desc = [x + ": " + get_entity_type_desc('conll2003')[x] for x in entity_set] # ATTENTION! dataset
    query, score = get_topk(recall_model, item["text"], entity_desc, len(entity_desc), 0.8) # threshold
    query = [x.split(": ")[0] for x in query]
    input = "List all named entities of the type [{}] and give explanations.\nText: {}\nEntities of type [{}] may exist in text".format(", ".join(entity_set), item["text"], ", ".join(query))
    target = []
...
def process_2_t5_type(data_path, data_type):
    sample_id = 0
    tag_map = get_tag_map(data_type)
    data_split = ["train", "validation", 'test'] # need your explanation dataset for train split
    out_files = ["train_recall_0.8_cls_exp.json", "valid_recall_0.8_cls_exp.json", "test_recall_0.8_cls_exp.json"] # output data name
    try:
...
if __name__ == "__main__":
    ...
    process_2_t5_type(data_path, data_type)
